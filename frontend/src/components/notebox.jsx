  ...
  const showLinks = () => {
    setModalContent(
      <div className="flex flex-col gap-2 p-2 justify-center items-center">
        {noteObj.refUrl.map((url, i) => (
          <UrlArea draft={noteObj.draft} key={i} url={url} />
        ))}
      </div>
    );
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`sm:max-w-80 max-w-[350px] w-full flex flex-col p-5 h-full min-h-52 max-h-72 ${lightColor} ${
          keepTheLatestNotesHidden && delay === 0 && "invisible"
        } rounded-3xl noteBox`}
      >
        <div className="h-[90%]">
          {isEditing ? (
            ...
            <>
              ...
              {keepTheLatestNotesHidden && delay === 0 ? (
                ...
                <motion.p
                  initial={{ y: -10, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                  className="text-stone-900 text-xl text-opacity-90 "
                >
                  {noteObj.content}
                </motion.p>
              )}
        ...
        <div className="flex justify-between items-center">
          {keepTheLatestNotesHidden && delay === 0 ? (
            ...
            <motion.p
              initial={{ y: -10, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              className="text-stone-800 text-lg text-opacity-75"
            >
              {noteObj.createdDate}
            </motion.p>
          )}
...
const AddNewUrlArea = () => {
  ...
  const handleSave = () => {};
  return (
    <div className="flex w-full flex-this.start gap-4 items-center">
      <input
  ...
  return (
    <div className="flex justify-between p-1 w-full">
      ...
      <div className="flex gap-1">
        {!isEditing ? (
          <EditIcon
            className="cursor-pointer text-midYellowLight active:scale-90 ease-linear duration-75"
            onClick={() => setIsEditing(true)}
          />
        ) : (
          <SaveIcon
